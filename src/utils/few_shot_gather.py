...
class Generator:
    ...
    async def generate(self, data_type: DataType) -> OutputFormat:
        async with self.semaphore:  # 控制并发
            ...
            self.logger.info(f"Processing patient {id}, tokens: {self.tokenizer_instance.count_tokens(hint_prompt)}")
            ...
            for loop_idx in range(self.max_loop):
                ...
                self.logger.info(f"Loop {loop_idx + 1} glean for patient {id}")
...
async def process_single_patient(generator, data, output_file):
    ...
    try:
        ...
        logging.error(f"Error processing patient {data.get('患者序号', 'unknown')}: {e}")
        return False
